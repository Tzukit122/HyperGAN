...
def discriminator(gan, config, x, g, xs, gs, prefix='d_'):
    ...
    if strided:
        net = conv2d(net, config.first_strided_conv_size, name=prefix+'_expand', k_h=7, k_w=min(7, gan.config.x_dims[1]), d_w=min(7, gan.config.x_dims[1]), d_h=7,regularizer=None,gain=config.orthogonal_initializer_gain)
    else:
    ...
    for i in range(depth):
      ...
      net = conv2d(net, int(int(net.get_shape()[3])*depth_increase), name=prefix+'_expand_layer'+str(i), k_w=min(3, gan.config.x_dims[1]), k_h=3, d_h=1, d_w=1, regularizer=None,gain=config.orthogonal_initializer_gain)
