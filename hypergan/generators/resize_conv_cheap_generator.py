...
def create(config, gan, net, input=None):
    ...
    for i in range(depth):
        ...
        for j in range(config.extra_layers):
            ...
            print("[extra generator layer]", net)
        name= 'g_layers_end'
        net = config.block(net, config, activation, batch_size, 'identity', 'g_laendyers_'+str(i), output_channels=gan.config.channels, filter=3, sigmoid_gate=sigmoid_gate)
